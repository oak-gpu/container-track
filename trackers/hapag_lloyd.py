...
_EVENTS_URL = "https://api.api-portal.hlag.com/hlag-tnt/v2/events"


class HapagLloydTracker(BaseTracker):
    def track(self, container_id: str) -> dict:
        if not config.HAPAG_LLOYD_API_KEY:
            return {"status": "ERROR: Hapag-Lloyd API key not configured", "location": ""}
        try:
            resp = requests.get(
                _EVENTS_URL,
                params={"equipmentReference": container_id},
                headers={
                    "Authorization": f"Bearer {config.HAPAG_LLOYD_API_KEY}",
...
def _parse(data) -> dict:
    # DCSA v2.2.4 response: list of event objects
